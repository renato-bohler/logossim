...
const adjustLink = link => {
  if (link.getSourcePort()) link.getSourcePort().addLink(link);

  if (link.getTargetPort()) link.getTargetPort().addLink(link);
...
const commandHandlers = engine => ({
  ...
  componentsAdded: ({ nodes }) => {
    engine.commands.add({
      ...
      undo: () => nodes.forEach(node => node.remove()),
    });
  },
  ...
  linkChanged: ({ before, after }) => {
    ...
    engine.commands.add({
      execute: () => handleLinkChanged(after, before),
      undo: () => handleLinkChanged(before, after),
    });
  },
  ...
  entitiesMoved: ({ nodes, links }) => {
    ...
    engine.commands.add({
      execute: () => handleEntitiesMoved('after'),
      undo: () => handleEntitiesMoved('before'),
    });
  },
});
